...
static constexpr uint16_t _getUtf16(uint8_t upper, uint8_t lower, bool be) {
	if (be) {
...
std::string decodeUcs2(const std::string &data, bool be) {
	if ((data.size() % 2) != 0) {
		// Invalid size
		return "";
	}
	...
	for (size_t i = 0; i < data.size(); i += 2) {
		uint16_t W1 = _getUtf16(data[i], data[i + 1], be);
		uint32_t value;
		...
		if (W1 >= 0xD800 && W1 <= 0xDBFF) {
			...
			uint32_t W2 = _getUtf16(data[i + 2], data[i + 3], be);
